 ...
 * Based on pipeline specification from webglpipeline.txt
 * Integrates with WebGL Fluid Simulation
 */
...
// ============ CONFIGURATION ============
const config = {
    // Fluid simulation settings (from original)
    SIM_RESOLUTION: 128,
    DYE_RESOLUTION: 1024,
    ...
    ANIMATE_NOISE: true,
    NOISE_SPEED: 0.05, // Slower motion for better stability
};

// Glass cards array - user can add cards here
const glassCards = [
    { x: 0.5, y: 0.5, width: 0.4, height: 0.2, cornerRadius: 0.03 }
];
...
// ============ FRAMEBUFFERS ============
let dye, velocity, divergence, curl, pressure, noiseFBO, blurFBO1, blurFBO2, fluidFBO;
...
function initFramebuffers() {
    ...
    // Glass effect FBOs
    let noiseRes = config.NOISE_RESOLUTION;
...
function pointerPrototype() {
    ...
    this.moved = false; this.color = { r: 0.3, g: 0.5, b: 0.8 };
}
...
function updatePointerMoveData(pointer, posX, posY) {
    ...
    // Always trigger move if deltas exist
...
canvas.addEventListener('mousemove', e => {
    // UPDATED: Allow splating on hover without mouse down
...
function renderGlassEffect() {
    ...
    gl.uniform3f(glassCompositeProgram.uniforms.uColorTint, config.COLOR_TINT.r, config.COLOR_TINT.g, config.COLOR_TINT.b);
...
function render() {
    if (!config.GLASS_ENABLED) {
        ...
        if (!config.TRANSPARENT) {
            colorProgram.bind();
            let c = normalizeColor(config.BACK_COLOR);
...
window.LiquidGlass = {
    ...
    setCardPosition: function (index, x, y) {
        if (glassCards[index]) {
            glassCards[index].x = x;
            glassCards[index].y = y;
        }
    }
};
...
console.log('WebGL Liquid Glass initialized. Use window.LiquidGlass API to add/manage glass cards.');
