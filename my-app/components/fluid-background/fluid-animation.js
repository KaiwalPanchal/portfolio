...
import { fluidConfig as defaultConfig, interactionConfig } from './config'
...
export default class FluidAnimation {
    ...
    onMouseMove = (e) => {
        // If hover enabled, treat as mouse down for movement purposes
        const isDown = interactionConfig.hoverEnabled ? true : this._pointers[0].down;
        this._pointers[0].moved = isDown;
        ...
        // Ensure color is set if it hasn't been (e.g. if we never clicked)
        // Check against initial placeholder value [30, 0, 300]
        if (interactionConfig.hoverEnabled && this._pointers[0].color[0] === 30) {
    ...
    _splat(x, y, dx, dy, color) {
        ...
        gl.uniform3f(this._programs.splat.uniforms.color, color[0] * interactionConfig.colorIntensity, color[1] * interactionConfig.colorIntensity, color[2] * interactionConfig.colorIntensity)
