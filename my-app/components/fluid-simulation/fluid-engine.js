/* eslint-disable */
// Ported from WebGL-Fluid-Simulation
// Wrapped for React
...
export function startFluidSimulation(canvas) {
    ...
    // Capture screenshot function omitted for brevity unless requested
...
`);
    ...
    const glassCompositeShader = compileShader(gl.FRAGMENT_SHADER, `
        precision highp float;
        precision highp sampler2D;
        varying vec2 vUv;
...
void main() {
    ...
    if (glassMask < 0.01) {
        discard; // Important for multiple cards
    }
    ...
    // We only draw glass, no background mixing needed since we discard outside
    gl_FragColor = vec4(color, 1.0 * glassMask);
}
`);
...
`);
    ...
    function initFramebuffers() {
        ...
        initGlassFramebuffers();
    }

    function initGlassFramebuffers() {
        let dyeRes = getResolution(config.DYE_RESOLUTION);
        const texType = ext.halfFloatTexType;
        const rgba = ext.formatRGBA;
        const filtering = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST;

        let noiseRes = config.NOISE_RESOLUTION || 512;
    ...
    function render(target) {
        ...
        // 1. Draw background to screen (normal fluid)
    ...
    function generateNoise() {
        ...
        gl.uniform1f(noiseProgram.uniforms.uScale, config.NOISE_SCALE || 2.0);
        gl.uniform1i(noiseProgram.uniforms.uOctaves, config.NOISE_OCTAVES || 4);
        gl.uniform1f(noiseProgram.uniforms.uNoiseSpeed, config.NOISE_SPEED || 0.05);
    ...
    function renderFluidToTexture() {
        ...
        // We could enable bloom/sunrays here for the glass too, but base fluid is often enough
        if (config.BLOOM) {
            gl.uniform1i(displayMaterial.uniforms.uBloom, bloom.attach(1));
            gl.uniform1i(displayMaterial.uniforms.uDithering, ditheringTexture.attach(2));
            let scale = getTextureScale(ditheringTexture, fluidFBO.width, fluidFBO.height);
            gl.uniform2f(displayMaterial.uniforms.ditherScale, scale.x, scale.y);
        }
        blit(fluidFBO);
    }

    function applyGaussianBlur(source, temp, dest, radius) {
        // Horizontal pass
        ...
        gl.uniform1f(gaussianBlurProgram.uniforms.uRadius, radius || 8.0);
        blit(temp);
    ...
    function renderGlassEffect() {
        ...
        gl.uniform1f(glassCompositeProgram.uniforms.uDisplacementScale, config.DISPLACEMENT_SCALE || 0.08);
        gl.uniform1f(glassCompositeProgram.uniforms.uChromaticAberration, config.CHROMATIC_ABERRATION || 0.015);
        gl.uniform1f(glassCompositeProgram.uniforms.uFresnelStrength, config.FRESNEL_STRENGTH || 0.4);
        gl.uniform1f(glassCompositeProgram.uniforms.uEdgeLightIntensity, config.EDGE_LIGHT_INTENSITY || 0.5);
        gl.uniform1f(glassCompositeProgram.uniforms.uSpecularIntensity, config.SPECULAR_INTENSITY || 0.4);
        gl.uniform1f(glassCompositeProgram.uniforms.uSpecularSize, config.SPECULAR_SIZE || 0.2);
        gl.uniform1f(glassCompositeProgram.uniforms.uBrightness, config.BRIGHTNESS || 0.02);
        gl.uniform1f(glassCompositeProgram.uniforms.uContrast, config.CONTRAST || 1.05);
        gl.uniform1f(glassCompositeProgram.uniforms.uSaturation, config.SATURATION || 1.4);
        gl.uniform3f(glassCompositeProgram.uniforms.uColorTint,
            config.COLOR_TINT?.r || 1.0,
            config.COLOR_TINT?.g || 1.0,
            config.COLOR_TINT?.b || 1.05
        );
    ...
    function handleMouseMove(e) {
        ...
        // Always treat as active for hover effect
        // if (!pointer.down) return; 

        // Use client coordinates which work regardless of what element is targeted
        let posX = scaleByPixelRatio(e.clientX);
        let posY = scaleByPixelRatio(e.clientY);

        // If the pointer wasn't "down" (initialized), allow it to initialize here too
        if (!pointer.down) {
            updatePointerDownData(pointer, -1, posX, posY);
        }
    ...
    function handleTouchStart(e) {
        // Do not prevent default to allow scrolling on mobile
        // e.preventDefault(); 
    ...
    return {
        ...
        removeCard(id) {
            glassCards = glassCards.filter(c => c.id !== id);
        },
        updateCard(id, options) {
            const card = glassCards.find(c => c.id === id);
            if (card) {
